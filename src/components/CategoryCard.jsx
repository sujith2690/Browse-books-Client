...
const CategoryCard = ({ categories }) => {
    ...
    const generateImage = async (category) => {
        try {
            const response = await axios.get(`https://source.unsplash.com/200x300/?${category}`, {
                responseType: 'blob',
            });
                ...
                <div key={i} className="flex flex-col justify-center items-center max-w-sm mx-auto my-8">
                    ...
                    <div className="w-56 md:w-64 bg-white -mt-10 shadow-lg rounded-lg overflow-hidden">
                        ...
                        <div className="flex items-center justify-between bg-indigo-500">
                            <Link
                                to={`/category/${item}`}
                                className="bg-indigo-500 text-xs text-white w-full px-2 py-1 font-semibold uppercase hover:bg-indigo-700 text-center"
                            >
                                Browse
                            </Link>
                        </div>
